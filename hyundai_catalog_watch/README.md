...
- 🎟️ **쿠폰 적용/해제 전환** (RRL 제품에 회원쿠폰이 '미적용↔적용'으로 바뀔 때 — 적용=🎟️ / 해제=🚫, 양방향). 쿠폰은 **로그인 상세페이지의 '상품 쿠폰 할인' 섹션**(매니아 % 쿠폰)에만 뜸 → 사이즈 스윕 때 각 제품 페이지에서 파싱. 브랜드 공통이라 **재고있는 대표 1개**의 상태로 판정해 전환 시 **1회만** 알림(state top-level `rrl_coupon`)

알림: **텔레그램은 이벤트별**(제품 URL 유용), **이메일은 카테고리별 통합**(재고 이벤트=1통, 쿠폰 이벤트=별도 1통). 첫 실행은 baseline(무알림).
...
3. **diff → 알림** — `state.json`(이전 스냅샷)과 비교. 위 이벤트 발생 시 텔레그램+이메일.
