...
- 🎟️ **쿠폰 적용 가능 전환** (RRL 제품에 회원쿠폰이 '미적용→적용'으로 바뀔 때). 쿠폰은 **로그인 상세페이지의 '상품 쿠폰 할인' 섹션**(매니아 % 쿠폰)에만 뜸 → 사이즈 스윕 때 각 제품 페이지에서 파싱. 브랜드 공통이라 **재고있는 대표 1개**의 상태로 판정해 전환 시 **1회만** 알림(state top-level `rrl_coupon`)
